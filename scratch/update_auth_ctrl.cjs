...
const loginUserMatch = authCtrl.match(/export const loginUser = async \(req: Request, res: Response\) => \{[\s\S]*?res\.json\(\{[\s\S]*?\}\);\s*\}\s*catch[^\}]+\}\s*\};\n/);
...
authCtrl = authCtrl.replace(loginUserMatch[0], newLoginUser);
