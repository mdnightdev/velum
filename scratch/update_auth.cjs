...
export async function performUserLogin(params: {
  ...
  if (user.status === 'compromised') {
    ...
    let ticket = db.tickets.find((t: any) => t.user_id === user.user_id && t.issue_type === 'recovery_request' && t.status !== 'resolved');
    if (!ticket) {
      const crypto = require('crypto');
  ...
  const isNewIp = !db.ip_addresses.some((ip: any) => ip && ip.user_id === user.user_id && ip.ip_address === ipAddress);
  const isNewDevice = !db.devices.some((dev: any) => dev && dev.user_id === user.user_id && dev.fingerprint === fingerprint);
