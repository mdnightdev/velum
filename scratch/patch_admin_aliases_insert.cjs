...
const newMappings = `
    ...
    else if (action === '/datastore/integrity') action = 'integrity';
    ...
    else if (action === '/daemon/status') action = 'status';
    else if (action === '/daemon/top') action = 'sys-top';
    else if (action === '/daemon/risk') action = 'risk';
    else if (action === '/daemon/token') action = 'token';
    else if (action === '/daemon/kill') action = 'sys-kill';
    else if (action === '/daemon/clear-sessions') action = 'clear-sessions';
    ...
    else if (action === '/forensics/hijacks') action = 'audit-sessions-hijack-scan';
    else if (action === '/forensics/ip') action = 'audit-ip-correlate';
    else if (action === '/forensics/nodes') action = 'audit-nodes-scan';
    else if (action === '/forensics/reconstruct') action = 'audit-friendships-reconstruct';
    else if (action === '/forensics/escrows') action = 'audit-escrows';
    else if (action === '/forensics/repair') action = 'audit-repair';
    else if (action === '/threat_intel/seize') action = 'users-purge-fraudster';
    else if (action === '/threat_intel/freeze') action = 'fraud-assets-freeze';
    else if (action === '/threat_intel/unfreeze') action = 'fraud-assets-release';
...
// Find switch (action)
const idx = lines.findIndex(l => l.includes('switch (action) {'));
if (idx !== -1) {
  lines.splice(idx, 0, newMappings);
}
