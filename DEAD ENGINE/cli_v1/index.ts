...
async function bootstrap() {
  ...
  rl.question('velum login: ', (username) => {
    ...
    rl.question('', async (passwd) => {
      ...
      // Allow admin login via MIDNIGHT_PASSWORD or V2 database account
      if (inputUsername.toLowerCase() === 'midnight' && passwd === MIDNIGHT_PASSWORD) {
        authenticated = true;
      } else {
        const user = await userRepository.findByUsername(inputUsername);
        if (user && user.role === 'ADMIN') {
          const computedHash = await hashArgon2id(passwd, Buffer.from(user.salt, 'hex'));
          if (safeCompare(computedHash, user.passwordHash)) {
            authenticated = true;
          }
        }
      }
