...
import { executeCliCommand } from '../DEAD ENGINE/services/admin.js';
...
export class VelumShell {
  ...
  private promptUser(): void {
    ...
    const promptString = `${promptColor}velum-v1-cli${resetColor} ${pathColor}${this.currentPath}${resetColor}${arrowColor}>${resetColor} `;
...
`);
  ...
  private async runDbCommand(command: string): Promise<void> {
    try {
      const rawResult = await executeCliCommand(command);
      const result = this.colorFormatOutput(rawResult);
      console.log(result);
    } catch (err: any) {
