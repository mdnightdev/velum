...
// CRITICAL: without this, an idle-connection error (Neon dropping a connection,
// a network blip, etc.) emits an unhandled 'error' event on the pool and takes
// down the entire Node process instantly — including any unsaved in-flight data.
pool.on('error', (err) => {
  console.error('[DB] Unexpected Postgres pool error (connection issue absorbed, server NOT crashing):', err);
});
