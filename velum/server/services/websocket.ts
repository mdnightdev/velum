...
wss.on('connection', (ws, req) => {
  ...
  console.log(`WebSocket connected: User ${userId} (Total clients: ${connectedClients.length})`);
      ...
      if (payload.type === 'join_room') {
        ...
        if (roomId.startsWith('ticket_')) {
          ...
          if (!isLounge) {
            ...
            const isAuthorized = parts.includes(String(userId)) || (roomId.startsWith('dm_velum_') && roomId.replace('dm_velum_', '') === String(userId));
            if (!isAuthorized) {
  ...
  ws.on('close', () => {
    ...
    console.log(`WebSocket disconnected. Total clients remaining: ${connectedClients.length}`);
  });
});
