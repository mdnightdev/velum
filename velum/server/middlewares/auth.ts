...
export const authenticateUser = (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const sess = (db.sessions || []).find((s) => s && (s.session_id === hashedSessionId || s.session_id === sessionId));
    if (!sess) {
...
export const verifySessionToken = (token: string): { session_id: string } | null => {
  ...
  const sess = (db.sessions || []).find(s => s && (s.session_id === hashedSessionId || s.session_id === token) && s.status === 'active');
  if (!sess) return null;
