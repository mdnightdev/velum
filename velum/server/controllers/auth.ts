...
import { validateCredentials, executePanicWipe, createNewSession } from '../services/authService.js';
...
export const registerUser = async (req: Request, res: Response) => {
  try {
    const { username, password, safeWord, panicPhrase, inviteCode, deviceFingerprint, recoveryKeyHash, recoveryKey, salt: clientSalt, plainRecoveryKey } = req.body;
    ...
    if (existingUser) {
      return res.status(400).json({ error: 'Username handle already taken.' });
    }
