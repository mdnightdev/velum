...
export const broadcastMessage = async (req: Request, res: Response) => {
  try {
    ...
    res.json({ success: true, message: 'Broadcast dispatched to individual channels.' });
  } catch (err: any) {
...
export const deleteUser = async (req: Request, res: Response) => {
  try {
    ...
    db.audit_logs.push({
      ...
      reason: `User @${targetUser.username} permanently deleted from state by admin "${admin.username}".`,
      timestamp: new Date().toISOString()
    });

    saveDb();
    res.json({ success: true, message: `User @${targetUser.username} successfully deleted and purged.` });
  } catch (err: any) {
...
export const approveRecovery = async (req: Request, res: Response) => {
  try {
    ...
    if (action === 'approve') {
      ...
      const tempCode = `LGN-RESTORE-${Math.floor(100000 + Math.random() * 900000)}`;
...
export const getDiagnostics = async (req: Request, res: Response) => {
  try {
    ...
    res.json({
      users: filteredUsers.map(u => ({ user_id: u.user_id, username: u.username, role: u.role, status: u.status })),
...
export const nominateSupport = async (req: Request, res: Response) => {
  try {
    ...
    if (admin.role !== 'LOGIN_ADMIN') {
      return res.status(403).json({ error: 'FAIL: Only EXECUTIVE Login Admins can nominate Support Operators.' });
    }
