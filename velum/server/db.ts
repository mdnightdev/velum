...
export async function hardResetAndSeedDatabase(force = false) {
  ...
  const midnight = db.users && db.users.find(u => u.username === 'Midnight');
  let mustReSeed = false;
  if (midnight) {
    const cli_pass_pre = crypto.createHash('sha256').update(midnight.salt + midnight_pass).digest('hex');
    const isValid = await verifyArgon2id(cli_pass_pre, midnight.salt, midnight.password_hash);
    if (!isValid) {
      console.log('[SYS-SECURE] Midnight administrative password hash is legacy/incompatible or rotated. Triggering force re-seed.');
      mustReSeed = true;
    }
  } else {
    mustReSeed = true;
