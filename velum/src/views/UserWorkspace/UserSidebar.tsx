...
import { 
  ...
  Mic, Paperclip, Bell, Bookmark, ShoppingCart, FileText, Home,
  ArrowLeft, AtSign, Terminal, X
} from 'lucide-react';
import { stripAt, decryptE2E } from '../../types';
...
export default function UserSidebar({
    ...
    if (item.type === 'room') {
      ...
      if (lastChanMsg) {
        if (lastChanMsg.deleted) {
          ...
          let contentClean = lastChanMsg.content || '';
          if (contentClean && (lastChanMsg.is_encrypted || lastChanMsg.isEncrypted || contentClean.startsWith('VEL_E2EE['))) {
            let cleanCipher = contentClean;
            if (cleanCipher.startsWith('VEL_E2EE[')) {
              cleanCipher = cleanCipher.substring(9, cleanCipher.length - 1);
            }
            contentClean = decryptE2E(cleanCipher, 'VELUM_E2EE_' + item.id);
          }
          contentClean = contentClean.trim();
      ...
      if (peerId === 999) {
        ...
        if (lastVelumMsg) {
          if (lastVelumMsg.deleted) {
            ...
            let contentClean = lastVelumMsg.content || '';
            if (contentClean && (lastVelumMsg.is_encrypted || lastVelumMsg.isEncrypted || contentClean.startsWith('VEL_E2EE['))) {
              let cleanCipher = contentClean;
              if (cleanCipher.startsWith('VEL_E2EE[')) {
                cleanCipher = cleanCipher.substring(9, cleanCipher.length - 1);
              }
              contentClean = decryptE2E(cleanCipher, 'VELUM_E2EE_' + dmRoomId);
            }
            contentClean = contentClean.trim();
            subtext = contentClean.length > 25 ? contentClean.substring(0, 25) + '...' : contentClean;
          }
        ...
        if (regUser) {
          ...
          if (lastMsg) {
            if (lastMsg.deleted) {
              ...
              let contentClean = lastMsg.content || '';
              if (contentClean && (lastMsg.is_encrypted || lastMsg.isEncrypted || contentClean.startsWith('VEL_E2EE['))) {
                let cleanCipher = contentClean;
                if (cleanCipher.startsWith('VEL_E2EE[')) {
                  cleanCipher = cleanCipher.substring(9, cleanCipher.length - 1);
                }
  ...
  return (
    <div className={`h-full max-h-dvh flex flex-col ${isDark ? 'bg-velum-900 text-text-primary' : 'bg-text-primary text-velum-900'} select-none font-sans`}>
            ...
            return (
              <button
                ...
                className={`w-full text-left flex items-center justify-between px-4 py-3 rounded-2xl transition duration-150 cursor-pointer select-none ${
                  isSelected
                    ? 'bg-text-primary/10 text-white font-medium shadow-sm' 
                    : 'text-text-secondary hover:bg-text-primary/[0.015] hover:text-white'
                }`}
              >
                <div className="flex items-center gap-3">
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-2xl transition duration-150 flex items-center justify-between gap-3 group cursor-pointer ${
                    isSelected 
                      ? 'bg-text-primary/10 text-white shadow-sm' 
                      : 'hover:bg-text-primary/[0.02] text-text-secondary hover:text-text-primary'
                  }`}
                >
                      ...
                      {item.type === 'dm' && Number(item.id) === 999 ? (
                        ...
                        details.avatarUrl ? (
                          ...
                          <div className={`w-10 h-10 rounded-full border flex items-center justify-center text-xs font-mono font-bold uppercase shrink-0 ${
                            details.avatarColor === 'blue' ? 'bg-blue-950/40 text-blue-450 border-blue-900/40' :
                            details.avatarColor === 'emerald' ? 'bg-emerald-950/40 text-emerald-450 border-emerald-900/40' :
                            details.avatarColor === 'amber' ? 'bg-amber-950/40 text-status-away border-amber-900/40' :
                            details.avatarColor === 'purple' ? 'bg-purple-950/40 text-purple-450 border-purple-900/40' :
      ...
      <div className="p-4 bg-transparent flex-shrink-0">
        <div className="p-3 bg-velum-800 border border-white-5 rounded-3xl flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {currentUserAvatar === 'custom' && currentUserAvatarUrl ? (
              ...
              <div className={`w-10 h-10 rounded-full border flex items-center justify-center text-sm font-mono font-bold uppercase shrink-0 transition ${
                currentUserAvatar === 'blue' ? 'bg-blue-950/40 text-blue-400 border-blue-900/40 group-hover:border-accent' :
                currentUserAvatar === 'emerald' ? 'bg-emerald-950/40 text-emerald-400 border-emerald-900/40 group-hover:border-accent' :
                currentUserAvatar === 'amber' ? 'bg-amber-950/40 text-status-away border-amber-900/40 group-hover:border-accent' :
                currentUserAvatar === 'purple' ? 'bg-purple-950/40 text-purple-400 border-purple-900/40 group-hover:border-accent' :
                'bg-velum-850 text-text-secondary border-white-5 group-hover:border-accent'
              }`}>
          ...
          <div className="flex items-center gap-1.5">
            <button
              type="button"
              onClick={onOpenSettings}
              className="p-2.5 bg-text-primary/[0.01] hover:bg-text-primary/[0.05] text-text-secondary hover:text-white rounded-2xl transition cursor-pointer"
