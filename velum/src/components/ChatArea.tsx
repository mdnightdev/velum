...
export default function ChatArea({
  ...
  const handleDeleteConversation = async () => {
    ...
    const dmRoomId = `dm_${Math.min(currentUserId, otherId)}_${Math.max(currentUserId, otherId)}`;

    try {
      const res = await fetch(`/api/friends/conversations/${dmRoomId}`, {
        method: 'DELETE',
        headers
      });
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    const unreadMessages = conversationMessages.filter(
      m => m.user_id !== currentUserId && m.status !== 'read'
    );
    
    unreadMessages.forEach(m => {
      onMarkAsRead(m.message_id, m.room_id || roomId);
    });
  }, [conversationMessages, currentUserId, roomId, onMarkAsRead]);
  ...
  return (
    <div className={`flex-1 flex flex-col overflow-hidden ${isDark ? 'bg-velum-900 text-text-primary' : 'bg-text-primary text-text-disabled'}`}>
          {/* Upper Navigation Bar */}
      <div className={`px-4 py-3 border-b flex items-center justify-between flex-shrink-0 bg-black border-white-5`}>
        {!wsConnected && (
                      ...
                      {isAttachment && (
                        <div className="mb-2.5">
                          {parsedAttachmentData && parsedAttachmentType.startsWith('image/') ? (
                            ...
                            <div className="flex items-center gap-3 p-3 bg-black/10 border border-white-5 rounded-xl mb-2.5 select-none text-left cursor-pointer hover:bg-black/20 transition"
                                 onClick={() => {
                            ...
                            <div className="flex items-center gap-3 p-3 bg-black/10 border border-white-5 rounded-xl mb-2.5 select-none text-left">
                              <div className="w-8 h-8 rounded-lg bg-accent/10 text-accent flex items-center justify-center shrink-0">
                                <FileIcon className="w-4 h-4" />
                              </div>
                              <div className="flex-1 min-w-0">
      ...
      <div className={`px-4 pb-4 pt-2 flex-shrink-0 bg-black`}>
        {micError && (
