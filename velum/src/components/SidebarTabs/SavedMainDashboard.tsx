...
export default function SavedMainDashboard({
  ...
  return (
    <div id="saved_vault_dashboard" className="flex-1 overflow-y-auto bg-velum-900 p-6 lg:p-8 space-y-6 select-none">
      <div>
        <h1 className="text-xs font-semibold uppercase tracking-widest text-text-primary">
          Local Encrypted Vault Notes
        </h1>
        <p className="text-[10px] text-text-secondary font-mono mt-0.5">
          ZERO KNOWLEDGE ENCLAVE STORAGE
        </p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        {/* Note Entry */}
        <form onSubmit={onSaveNote} className="lg:col-span-5 bg-velum-800 border border-white-5 rounded p-5 space-y-4">
          ...
          {savedNotes.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {savedNotes.map((note, idx) => (
                <div key={idx} className="bg-velum-800 border border-white-5 p-4 rounded flex flex-col justify-between space-y-3 shadow-md relative group">
