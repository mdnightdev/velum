...
import { handleCardsCommand } from './handlers/cardsHandler.js';
import { handleTicketsCommand } from './handlers/ticketsHandler.js';
import { handleSysCommand } from './handlers/sysHandler.js';
  ...
  public async runCommand(line: string): Promise<void> {
    ...
    if (fullCmd.startsWith('/')) {
      ...
      if (['list', 'view', 'create', 'deactivate', 'cancel', 'restore', 'pending', 'purge'].includes(fullCmd)) {
        ...
        ns = '/bank';
      } else if (['config', 'maint', 'fee', 'tax', 'rate', 'escrow-fee'].includes(fullCmd)) {
        ns = '/devops';
      } else if (['integrity', 'orphans', 'clean', 'vacuum', 'wipe', 'health', 'pg', 'redis'].includes(fullCmd)) {
        ns = '/db';
      } else if (['cards', 'credit', 'debit'].includes(fullCmd)) {
        ns = '/cards';
      } else if (['tickets'].includes(fullCmd)) {
        ns = '/tickets';
      }
    }

    // High risk actions simple y/n prompt
    const isHighRisk = sub === 'purge' || sub === 'wipe' || sub === 'clean' || sub === 'vacuum' || sub === 'purge-all';
    if (isHighRisk) {
    ...
    if (ns === '/cards') return handleCardsCommand(sub, rawArgs);
    if (ns === '/tickets') return handleTicketsCommand(sub, rawArgs);
    if (ns === '/sys') return handleSysCommand(sub, rawArgs);
