...
export const getBankAccounts = async (req: Request, res: Response) => {
  try {
    ...
    // Support admins only see Escrow accounts, never standard client details or central reserve
    if (user.role === 'SUPPORT_ADMIN') {
      const escrowAccounts = accounts.filter(
        a => a.account_id === 'bank_escrow_reserve' || a.account_name.includes('ESCROW')
      );
...
export const getBankTransactions = async (req: Request, res: Response) => {
  try {
    ...
    // Support admins can only see Escrow holds or releases
    if (user.role === 'SUPPORT_ADMIN') {
      const escrowTransactions = transactions.filter(
        t => t.account_id === 'bank_escrow_reserve' || t.type.startsWith('escrow')
      );
