  ...
  try {
    ...
    if (fs.existsSync(SQLITE_FILE)) {
      ...
      try {
        ...
        db.reports = loadPayloadTable('reports');
        ...
        db.user_wallets = loadPayloadTable('user_wallets');
        db.wallet_ledger_entries = loadPayloadTable('wallet_ledger_entries');
        db.recharge_requests = loadPayloadTable('recharge_requests');
        db.withdrawal_requests = loadPayloadTable('withdrawal_requests');
        db.kyc_verifications = loadPayloadTable('kyc_verifications');
        db.payment_methods = loadPayloadTable('payment_methods');
        db.external_financial_accounts = loadPayloadTable('external_financial_accounts');
        db.external_processor_events = loadPayloadTable('external_processor_events');
        db.wallet_balances = loadPayloadTable('wallet_balances');
        db.currencies = loadPayloadTable('currencies');
        db.exchange_rates = loadPayloadTable('exchange_rates');
        db.platform_admins = loadPayloadTable('platform_admins');

        // Marketplace Tables
        db.market_assets = loadPayloadTable('market_assets');
        db.market_sku_variants = loadPayloadTable('market_sku_variants');
        db.market_asset_media = loadPayloadTable('market_asset_media');
        db.market_reviews = loadPayloadTable('market_reviews');
        db.market_coupons = loadPayloadTable('market_coupons');
        db.market_discussions = loadPayloadTable('market_discussions');
        db.market_support_chats = loadPayloadTable('market_support_chats');
        db.listing_verification_checks = loadPayloadTable('listing_verification_checks');
