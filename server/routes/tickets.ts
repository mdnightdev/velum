...
ticketsRouter.post('/tickets', (req, res) => {
  const { username, issueType, disputeText, senderUserId, ipAddress, deviceFingerprint, safeWordEntered } = req.body;

  const targetUser = db.users.find(u => u.username === username) || db.users.find(u => u.user_id === senderUserId);
  ...
  const isReportType = issueType === 'compromise_report' || issueType === 'report_user' || issueType === 'system_bug' || issueType === 'suggestion' || issueType === 'user_misconduct' || issueType === 'bug_report';
  if (isReportType) {
    const reportId = `rep_${Date.now()}`;
    const isHigh = disputeText?.toLowerCase().includes('scam') || disputeText?.toLowerCase().includes('fraud');
    const newReport: Report = {
      report_id: reportId,
      reporter_id: uId,
      reporter_name: uName,
      type: issueType === 'system_bug' || issueType === 'bug_report' ? 'bug_report' : (issueType === 'suggestion' ? 'suggestion' : 'user_misconduct'),
      priority: isHigh ? 'HIGH' : 'LOW',
      reason: disputeText || 'Report filed.',
      status: 'pending',
