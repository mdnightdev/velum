...
export function getStepOTP(): string {
  const interval = Math.floor(Date.now() / 120000);
  const hash = crypto.createHash('sha256').update(`${interval}_velum_otp`).digest('hex');
...
export function checkStepOTP(token: string): boolean {
  ...
  for (let offset = -1; offset <= 1; offset++) {
    const val = currentInterval + offset;
    const hash = crypto.createHash('sha256').update(`${val}_velum_otp`).digest('hex');
    const numericValue = parseInt(hash.substring(0, 8), 16);
