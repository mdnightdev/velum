...
/** Listings may be priced in EUR (default) or VLM. */
export const MARKET_LISTING_CURRENCIES = ['EUR', 'VLM'] as const;
export type MarketListingCurrency = (typeof MARKET_LISTING_CURRENCIES)[number];
...
export const escrows = pgTable(
  'escrows',
  {
    ...
    sellerId: integer('seller_id')
      .references(() => users.id, { onDelete: 'cascade' })
      .notNull(),
    /** Settlement amount in listing currency (held for seller). */
    amount: numeric('amount', { precision: 18, scale: 2 }).notNull(),
    currency: varchar('currency', { length: 8 }).default('EUR').notNull(),
    /** Currency actually debited from buyer (may differ after FX). */
    paymentCurrency: varchar('payment_currency', { length: 8 }).default('EUR').notNull(),
