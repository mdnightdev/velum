...
class Logger {
  ...
  constructor(prefix: string, defaultLevel: LogLevel = 'info') {
    this.prefix = prefix;
  ...
  private resolveLevel(defaultLevel: LogLevel): LogLevel {
    const envLevel = (import.meta.env.VITE_LOG_LEVEL as LogLevel) || 
                     (import.meta.env.MODE === 'production' ? 'warn' : 'info');
...
export const createLogger = (prefix: string, defaultLevel?: LogLevel) => new Logger(prefix, defaultLevel);