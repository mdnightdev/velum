...
export default function EscrowTransactionCard({
  ...
  return (
    <div className="glass-card border border-emerald-900/30 p-5 space-y-4 relative overflow-hidden shadow-xl">
      <div className="flex justify-between items-start">
        ...
        <div className="text-right">
          ...
          <span className="text-lg font-mono font-black text-emerald-400">${Number(escrow.amount || 0).toFixed(2)}</span>
        </div>
      </div>
      ...
      <div className="flex justify-between text-[10px] font-mono bg-black/20 p-2.5 rounded-lg border border-white-5">
        <div>
          ...
          <span className="text-red-400 font-bold">${Number(escrow.platform_fee || 0).toFixed(2)}</span>
        </div>
        <div>
          <span className="text-text-secondary mr-2 text-right">Net Creator Settlement:</span>
          <span className="text-emerald-400 font-bold">${Number(escrow.payout_amount || 0).toFixed(2)}</span>
        </div>
      </div>
