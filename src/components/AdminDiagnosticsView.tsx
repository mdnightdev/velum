...
export default function AdminDiagnosticsView({
  ...
  const handleResolve = async (logId: string) => {
    try {
      let res: Response;
      if (adminFetch) {
        res = await adminFetch(`/v2/admin/diagnostics/logs/${logId}/resolve`, {
          method: 'POST',
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-5 rounded-xl border bg-velum-850/60 border-white-5">
          ...
          <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
            ...
            {suspicious.length === 0 && (
              <div className="text-text-disabled text-center py-16 font-mono text-[10.5px] font-black uppercase tracking-wider">No threats or suspicious activity detected</div>
            )}
          </div>
        </div>
        ...
        <div className="p-5 rounded-xl border bg-velum-850/60 border-white-5">
          ...
          <div className="space-y-3.5 max-h-[300px] overflow-y-auto pr-1">
            ...
            {logs.length === 0 && (
              <div className="text-text-disabled text-center py-16 font-mono text-[10.5px] font-black uppercase tracking-wider">No administrative audit logs recorded</div>
            )}
          </div>
        </div>
      </div>
            ...
            <div className="p-4 rounded-xl bg-velum-900 border border-white-10 font-mono text-xs space-y-3 overflow-y-auto max-h-[360px]">
              {selectedLog ? (
                ...
                <div className="text-text-disabled text-center py-20 uppercase tracking-wider text-[11px]">
                  Select a client diagnostic log to view details
                </div>
              )}
            </div>
          </div>
