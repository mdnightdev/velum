...
export function ListingInspector({
  ...
  const handleUpdatePrice = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await fetch(`/v2/marketplace/listings/${listing.listing_id}`, {
        method: 'PUT',
        headers: {
  ...
  const fetchDiscussions = async () => {
    ...
    try {
      const sId = fetchSessionId();
      const res = await fetch(`/v2/marketplace/listings/${listing.listing_id}/discussions`, {
  ...
  const fetchReviews = async () => {
    ...
    try {
      const sId = fetchSessionId();
      const res = await fetch(`/v2/marketplace/listings/${listing.listing_id}/reviews`, {
        headers: { 'Authorization': `Bearer ${sId}` }
      });
      if (res.ok) {
        const data = await res.json();
  ...
  const fetchMedia = async () => {
    ...
    try {
      const sId = fetchSessionId();
      const res = await fetch(`/v2/marketplace/listings/${listing.listing_id}/media`, {
        headers: { 'Authorization': `Bearer ${sId}` }
      });
      if (res.ok) {
        const data = await res.json();
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-velum-800 to-transparent p-5 pt-12 flex justify-between items-end gap-4">
            ...
            <div className="text-right shrink-0">
              ...
              <span className="text-sm sm:text-base font-mono font-black text-accent block">
                ${Number(currentTotalDisplayPrice || 0).toFixed(2)}
              </span>
              {selectedSku && (
                ...
                <div className="bg-velum-850/50 border border-white-5 p-4 rounded-2xl space-y-3 text-left">
                  ...
                  {isEditingPrice ? (
                    ...
                    <div className="text-xs text-text-secondary">
                      Current Price: <span className="text-white font-bold">${Number(listing.price || 0).toFixed(2)}</span>
                      {listing.discount_price !== undefined && listing.discount_price !== null && (
                        <span> (Discounted: <span className="text-accent font-bold">${Number(listing.discount_price || 0).toFixed(2)}</span>)</span>
                      )}
                    </div>
                  )}
                </div>
                    ...
                    {(listing.sku_variants || []).map((variant: MarketSkuVariant) => (
                      ...
                      >
                        ...
                        <div className="text-[9px] font-mono mt-1 text-accent font-black">
                          +${(Number(variant.additional_cost_cents || 0) / 100).toFixed(2)}
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                <div className="space-y-1.5 text-center sm:text-left">
                  ...
                  <div className="flex items-center gap-2 justify-center sm:justify-start">
                    <span className="text-xl font-mono font-black text-white">
                      {listing.average_rating && Number(listing.average_rating) > 0 ? Number(listing.average_rating).toFixed(1) : '0.0'}
                    </span>
