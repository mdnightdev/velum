...
import { BadgeCheck, Terminal, Radio, ShieldCheck, ShieldAlert } from 'lucide-react';
// @ts-ignore
...
export default function DashboardLayout({
  ...
  try {
    return (
      <div className={`${styles.root} ${isDark ? styles.dark : styles.light} overflow-hidden flex flex-col md:flex-row`}>
        <SettingsDrawer
        ...
        <main className={`${styles.main} ${isMobile ? 'pb-16' : ''} glass-panel border-y-0 border-r-0 rounded-none`}>
          {activeCategory === 'wallet' ? (
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col">
              {isMobile && (
                <div className="p-3 bg-velum-850 border-b border-white-5 flex items-center justify-between">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-accent">Marketplace</span>
                </div>
              )}
              <MarketMainDashboard
                currentUserId={user?.userId || 0}
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col">
              {isMobile && (
                <div className="p-3 bg-velum-850 border-b border-white-5 flex items-center justify-between">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-accent">Arbitration Support</span>
                </div>
              )}
            ...
            <div className="flex-1 overflow-hidden relative flex flex-col">
              {isMobile && (
                <div className="p-3 bg-velum-850 border-b border-white-5 flex items-center justify-between">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-accent">Vault Notes</span>
                </div>
              )}
              <SavedMainDashboard
            ...
            <div className="flex-grow flex-shrink flex-1 min-h-0 overflow-hidden relative flex flex-col">
              {isMobile && (
                <div className="p-3 bg-velum-850 border-b border-white-5 flex items-center justify-between">
                  <span className="text-[10px] font-mono uppercase tracking-widest text-accent">Notifications</span>
                </div>
              )}
              ...
              {activeLoungeId ? (
                <LoungeWorkspace
                  ...
                  onLoungeSelect={(lid, lname) => {
                    ...
                    onRoomSelect(''); // Clear selected room when changing lounge
                  }}
                  onBackToDirectory={() => {
                    setActiveLoungeId('');
                    setActiveLoungeName('');
          ...
          {isMobile && (
            <BottomBar
              ...
              activeRoomId={activeRoomId}
              activeChatPeer={activeChatPeer}
              pendingRequestsCount={friendRequests.filter(r => r && r.status === 'pending' && Number(r.receiver_id) === user?.userId).length}
