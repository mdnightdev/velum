...
import { getExtendedReactions, getQuickReactions } from '../../utils/reactions';
...
export function ReactionPicker({
  ...
  const quick = useMemo(() => availableReactions ?? getQuickReactions(), [availableReactions]);
  const extended = useMemo(() => getExtendedReactions(), []);

  return (
    <div
      data-reaction-picker="true"
      className={`absolute -top-14 bg-velum-850/98 backdrop-blur-xl border border-accent/30 p-1.5 rounded-2xl flex flex-col gap-2 shadow-2xl z-[100] transition-all animate-in fade-in zoom-in-95 duration-150 ${
        isMe ? 'right-0' : 'left-0'
      }`}
    >
      <div className="flex items-center gap-1">
        {quick.map((reaction) => (
          <button
            ...
            className="hover:scale-130 active:scale-95 transition-transform p-1.5 text-base cursor-pointer rounded-xl hover:bg-white-10 emoji-font"
