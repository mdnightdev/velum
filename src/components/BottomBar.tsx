...
import { Users, Globe, MessageSquare, User, ShoppingCart, Wallet } from 'lucide-react';
...
interface BottomBarProps {
  ...
  onSelectCategory: (cat: 'rooms' | 'direct' | 'market' | 'tickets' | 'saved' | 'people' | 'notifications' | 'wallet') => void;
  onRoomSelect: (roomId: string) => void;
  onClearChatPeer: () => void;
  onOpenSettings: () => void;
  onToggleSidebar: () => void;
  onTabClick?: (tabId: 'people' | 'lounge' | 'chats' | 'profile' | 'market' | 'wallet') => void;
}
...
export default function BottomBar({
  ...
  const getActiveTab = () => {
    ...
    if (activeCategory === 'wallet') return 'wallet';
    return '';
  };

  const activeTab = getActiveTab();

  const handleTabClick = (tabId: 'people' | 'lounge' | 'chats' | 'profile' | 'market' | 'wallet') => {
    if (onTabClick) {
      onTabClick(tabId);
      return;
    }
