...
export default function AdminBank({ adminRole, adminFetch }: AdminBankProps) {
  ...
  const fetchBankData = async () => {
    ...
    setBankError('');
    try {
      const accRes = await adminFetch('/v2/bank/accounts');
      if (accRes.ok) {
        const data = await accRes.json();
        setBankAccounts(data || []);
      }
      const txRes = await adminFetch('/v2/bank/transactions');
      if (txRes.ok) {
      ...
      const qRes = await adminFetch('/v2/bank/withdrawals');
      if (qRes.ok) {
        setWithdrawalQueue(await qRes.json() || []);
      }
      
      
      const lRes = await adminFetch('/v2/bank/limits');
      if (lRes.ok) {
        setLimitsData(await lRes.json() || []);
      }
      const cRes = await adminFetch('/v2/bank/issued-cards');
      if (cRes.ok) {
  ...
  const handleToggleFreeze = async (accountId: string, currentlyFrozen: boolean) => {
    setBankError('');
    try {
      const res = await adminFetch(`/v2/bank/accounts/${accountId}/freeze`, {
        method: 'POST',
  ...
  const totalLiquidity = safeBankAccounts.reduce((sum, acc) => sum + (acc.balance_cents / 100), 0);

  const oneDayAgo = Date.now() - 86400000;
  const recentTxs = safeBankTransactions.filter(tx => new Date(tx.timestamp).getTime() > oneDayAgo);
  const volume24h = recentTxs.reduce((sum, tx) => sum + tx.amount_cents, 0) / 100;
  const frozenCount = safeBankAccounts.filter(a => a.status === 'frozen').length;
  ...
  const handleReviewWithdrawal = async (requestId: string, action: 'APPROVE' | 'REJECT') => {
    ...
    try {
      ...
      const res = await adminFetch(`/v2/payments/withdraw/review`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
                      ...
                      <tr key={idx} className="hover:bg-white-2 transition-colors">
                        ...
                        <td className="px-6 py-5 text-right font-mono text-[13px] text-text-primary">
                          <div className="text-text-primary">
                            Limit: ${ Number(limit / 100 || 0).toFixed(2) }
                          </div>
                          <div className={`text-[11px] mt-1 ${usedAmount > 0 ? 'text-status-offline font-bold' : 'text-status-online'}`}>
                            Balance: ${ Number((card.available_cents - limit) / 100 || 0).toFixed(2) }
                          </div>
                        </td>
                      </tr>
                    )})}
