    ...
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3 border-b border-velum-600 pb-3">
        ...
        <div className="flex flex-wrap items-center gap-2 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1 bg-velum-750 border border-velum-600 p-0.5 rounded-xl">
            {[
              { value: 'all', label: 'All' },
              { value: 'CLI_ADMIN', label: formatRoleLabel('CLI_ADMIN') },
              { value: 'LOGIN_ADMIN', label: formatRoleLabel('LOGIN_ADMIN') },
              { value: 'SUPPORT_ADMIN', label: formatRoleLabel('SUPPORT_ADMIN') },
              { value: 'USER', label: formatRoleLabel('USER') },
              { value: 'BLOCKED', label: formatRoleLabel('BLOCKED') },
            ].map((opt) => (
              <button
                key={opt.value}
                type="button"
                onClick={() => setUserRoleFilter(opt.value as any)}
                className={`px-3 py-1.5 rounded-lg text-[10px] font-semibold transition-all cursor-pointer font-sans ${
                  userRoleFilter === opt.value
                    ? 'bg-accent/15 border border-accent/20 text-accent shadow-inner'
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-xs font-sans">
          <thead>
            <tr className="text-text-secondary text-[10px] font-semibold border-b border-white-5 text-left">
          ...
          <tbody className="divide-y divide-white/[0.02]">
            {[...(Array.isArray(users) ? users : [])]
              ...
              .map((u) => {
                ...
                const roleLabel = u.id === 999 ? 'System' : formatRoleLabel(u.role);
                ...
                return (
                  ...
                  >
                    ...
                    <td className="py-4">
                      <div className="flex items-center gap-2">
                        <span
                          className={`w-3.5 h-3.5 rounded-full ${rConf.bg} border ${rConf.border} flex items-center justify-center shrink-0`}
                          title={roleLabel}
                        >
                          <span className={`w-1.5 h-1.5 rounded-full ${rConf.text}`} />
                        </span>
                        <span className="text-text-primary text-[11px] font-medium">{roleLabel}</span>
                      </div>
                    </td>
                              ...
                              <button
                                onClick={async () => {
                                  try {
                                    ...
                                    if (res.ok) {
                                      velumToast.success(`Nominated @${u.username} for Support`);
                                      fetchData();
                                    } else {
                                      const errData = await res.json();
